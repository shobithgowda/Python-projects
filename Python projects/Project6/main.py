...
class CurrencyConverter:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.rates = None
            messagebox.showerror("Error", f"Failed to fetch exchange rates from the API: {e}")

    def convert(self, amount, base_currency, des_currency):
        if self.rates is None:
            return "API Error"
        if base_currency != 'EUR':
            amount = amount/self.rates.get(base_currency, 1)  # Use default value 1 if currency not found

        # Limiting the result to 2 decimal places
        amount = round(amount*self.rates.get(des_currency, 1), 2)  # Use default value 1 if currency not found
...
class Main(tk.Tk):
    ...
    def __init__(self, converter):
        ...
        if self.CurrencyConverter.rates:
            ...
            messagebox.showerror("Error", "Failed to fetch exchange rates from the API")
